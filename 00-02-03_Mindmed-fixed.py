...
####################### MT ###################
        ...
        self.b510_4.clicked.connect(self.savewellbeing)
...
############## Add New Employee ###########
    ...
    def fill_next_employee_id(self):
        try:
            employee_id = 0
            mydb = con.connect(host="localhost", user="root", password="kavish1202", db = "MindMed")
            cursor = mydb.cursor()
 ...
 ############################################### MT ######################################
    ...
    def saveanxiety(self):
        try:
            ...
            answer7 = self.tb817.text()
             ...
             #adding date of taking survey
            date_of_survey = datetime.date.today()

            #adding the answers to calculate the percentage
            total = str(((int(answer1) + int(answer2) + int(answer3) + int(answer4) + int(answer5) + int(answer6) + int(answer7))*100)/21) +"%"
            # date_of_adding_session = QDate.currentDate().toString(Qt.ISODate)

            # self.date_label.setText(date_of_adding_session) # Displays current date in label

            sql_insert_student = "insert into moodtracker (first_name,last_name,department,answer1,answer2,answer3,answer4,answer5,answer6,answer7,date_of_survey,total) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            values = (first_name, last_name, department, answer1, answer2, answer3, answer4, answer5, answer6, answer7, date_of_survey, total)
    ...
    def savewellbeing(self):
        try:
            ...
            #adding the answers to calculate the percentage
            total = str((int(answer1) + int(answer2) + int(answer3) + int(answer4) + int(answer5))*4) +"%"

            # date_of_adding_session = QDate.currentDate().toString(Qt.ISODate)

            # self.date_label.setText(date_of_adding_session) # Displays current date in label

            #added two values, date of survey and total to the db
            sql_insert_student = "insert into moodtrackerwho (first_name,last_name,department,answer1,answer2,answer3,answer4,answer5,date_of_survey,total) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            values = (first_name, last_name, department, answer1, answer2, answer3, answer4, answer5, date_of_survey, total)
